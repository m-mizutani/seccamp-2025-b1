...
#### 8:45-9:30 - クラウド環境固有のセキュリティ監視基盤
- **クラウド環境の特徴とセキュリティ課題** (20分)
  - 責任共有モデルの詳細理解
  - スケーラビリティと可視性のトレードオフ
  - 動的インフラの監視課題（Auto Scaling、コンテナ、サーバーレス）
  - マルチアカウント・マルチリージョン環境の複雑性

- **セキュリティデータレイク中心の監視アーキテクチャ** (25分)
  - **セキュリティデータ収集の全体像**
  ...
  - **データレイクアーキテクチャの基本設計**
    ...
    ├ VPC Flow Logs     ├ 形式統一      ├ OCSF準拠     ├ SQL(Athena)   ├ リアルタイム検知
    ├ DNS Logs          ├ 重複排除      ├ パーティション  ├ 可視化ツール   ├ バッチ分析  
    ├ CloudTrail        ├ 機密データ除去  ├ 圧縮最適化    └ 外部連携      └ レポート生成
    └ Application Logs  └ エンリッチメント └ 長期保存
    ```
  
  - **リアルタイム vs バッチ処理の使い分け**
    - ストリーミング：重大脅威の即座検知（侵入、マルウェア）
    - バッチ処理：パターン分析、トレンド検知（内部脅威、異常行動）
    - コスト効率と検知速度のバランス設計

### 9:30-9:35 (5分) - 休憩

#### 9:35-10:10 - クラウドセキュリティサービスとカスタム検知戦略

- **クラウドプラットフォーム共通のセキュリティサービス概念** (20分)
